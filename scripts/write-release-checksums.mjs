...
const releaseExtensions = new Set(['.exe', '.zip', '.yml', '.blockmap'])
...
export async function generateReleaseChecksums(directory, outputName = 'SHA256SUMS.txt') {
  const files = (await readdir(directory, { withFileTypes: true }))
    .filter((entry) => entry.isFile() && entry.name !== outputName && releaseExtensions.has(path.extname(entry.name).toLowerCase()))
