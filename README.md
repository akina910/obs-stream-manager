...
個人設定、ゲームプロファイル、サムネイル、BGMストック、共通配信テンプレート、説明文、ログ、バックアップは `%APPDATA%\obs-stream-manager` に保存されます。アプリを更新してもこのフォルダーは維持されます。BGMは1ファイル50 MBまでで、OBSには管理用メディアソース `BGM Stock` として追加されます。
