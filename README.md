...
`OBS Stream Manager-Portable-*-x64.exe` を任意の場所から直接起動します。インストールは不要です。OBSプラグインを自動配置するため、起動時にWindowsの管理者確認が表示されます。移動後の壊れた自動起動登録を避けるため、Portable版はWindowsの自動起動へ登録しません。OBSを開く前にEXEを起動してください。
...
OBS Stream Managerは、OBSが閉じている間に内蔵接続機能、接続パスワード、`Stream Manager`ドックを自動準備します。URLやパスワードの入力、WebSocket設定、カスタムブラウザドックの手動登録は不要です。OBSを起動中に初めてアプリを開いた場合だけ、OBSを一度終了して開き直してください。既存の他のドックとOBS設定は維持します。障害時の復旧情報だけ [OBS_SETUP.md](docs/OBS_SETUP.md) に分離しています。
...
同梱プラグインはWindowsの管理者確認後、OBS公式のWindows用プラグイン領域`C:\ProgramData\obs-studio\plugins\obs-stream-manager-output`へ自動配置されます。
...
個人設定、ゲームプロファイル、サムネイル、BGMストック、共通配信テンプレート、説明文、ログ、バックアップは `%APPDATA%\obs-stream-manager` に保存されます。アプリを更新してもこのフォルダーは維持されます。BGMは1ファイル50 MBまでで、OBSには管理用メディアソース `BGM Stock` として追加されます。設定画面から書き出すバックアップにはBGM本体と選択状態も含まれます（合計150 MBまで）。
