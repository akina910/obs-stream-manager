...
初回起動時はセットアップダイアログが開き、OBSドックURL、OBS WebSocket、YouTube／Twitch認証、ゲーム検出を順番に確認できます。完了状態は保存され、次回起動時には自動で閉じます。設定画面からいつでも開き直せます。

Steamは任意です。Steamが見つかった場合だけインストール済みゲームを自動追加し、Steamがない場合はGame Pass、GeForce NOW、Switch、単体EXEのゲームを手動追加して利用できます。ARKやDiabloなどの固定ゲームを初期状態へ勝手に追加しません。
...
4. アプリへ戻り、両サービスが「認証保存済み」になったことを確認します。
5. ゲームを選び、「配信開始」を押します。
...
「認証保存済み」は資格情報がWindowsへ保存された状態であり、実際の配信接続成功を意味しません。YouTube／Twitchそれぞれの実通信状態は上部の配信状態で確認してください。

> **開発中の制限:** 現行版はTwitchのタイトル・カテゴリ・タグをOAuthで自動更新できますが、Aitum MultistreamへTwitchの映像出力資格情報を安全に自動設定する機能は未完成です。YouTube／Twitch同時配信は、事前にAitum側の出力が設定済みの環境だけを対象にテストしてください。OAuth操作だけで新規環境の同時配信まで完結する状態は、まだリリース合格としていません。
