...
`OBS Stream Manager-Setup-*-x64.exe` を実行し、画面に従ってユーザー単位でインストールします。スタートメニューとデスクトップへショートカットを作成します。通常利用にはこちらを推奨します。初回起動後はWindowsログイン時に非表示で準備する設定が既定で有効になり、OBSドックをすぐ読み込めます。設定画面または通知領域のメニューから無効にできます。
...
> [!IMPORTANT]
> YouTube接続では配信枠、メタデータ、サムネイル、OBSへ適用する映像出力情報を管理します。Twitch接続ではタイトル、カテゴリ、タグ、コメント、実配信状態を管理しますが、Twitchの映像出力先は自動設定しません。YouTube／Twitch同時配信には、Aitum Multistream側のTwitch出力を事前に設定する必要があります。Twitchが `LIVE` になるまで同時配信成功と判断しないでください。

## 必要環境

- 対応対象: Windows 11 x64
- 対応対象: OBS Studio 30以上（内蔵WebSocketサーバーを有効化）
- YouTube／Twitch同時映像出力を行う場合: Aitum Multistream
- 任意: Aitum Vertical、Source Record、Advanced Scene Switcher、Steam

Aitum MultistreamがなくてもYouTube単独利用、ゲームプロファイル管理、通常のOBS操作は利用できます。Source RecordやAitum Verticalが無い場合は、それぞれの個別録画機能だけが警告付きで無効になります。
