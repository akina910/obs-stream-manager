...
export type SecretName = 'obs-password' | 'obs-applied-stream-service' | 'obs-previous-stream-service' | 'obs-output-mode-reload-required' | 'obs-recording-previous-profile' | 'steam-api-key' | 'youtube-client-secret' | 'youtube-refresh-token' | 'youtube-oauth-health' | 'youtube-stream-key' | 'youtube-stream-server' | 'youtube-api-limit-until' | 'youtube-pending-completion-id' | 'youtube-observed-active' | 'twitch-client-secret' | 'twitch-access-token' | 'twitch-refresh-token' | 'twitch-oauth-health' | 'twitch-stream-key' | 'twitch-stream-server'
