...
export class OAuthManager {
  private readonly states = new Map<string, { provider: Provider; expiresAt: number }>()

  constructor(private readonly store: DataStore, private readonly secrets: SecretStore, private readonly callbackOrigin: string) {}

  private state(provider: Provider): string {
    const state = crypto.randomBytes(24).toString('base64url')
    this.states.set(state, { provider, expiresAt: Date.now() + 10 * 60_000 })
    return state
  }
  ...
  async authorizationUrl(provider: Provider): Promise<string> {
  ...
  async exchange(provider: Provider, code: string, state: string): Promise<void> {
