...
function SettingsView({ config, status, oauthStatus, oauthProgress, steamScan, onSave, onBackup, onRestore, onSteamScan, onSteamSync, onOAuthConnect, onTwitchOutputTest, onReconnect, onOpenSetup }: { config: AppConfig; status: RuntimeStatus; oauthStatus: OAuthConnectionStatuses; oauthProgress: OAuthProgress; steamScan: SteamSyncResult | null; onSave: (config: AppConfig, secrets: Record<string, string>) => Promise<void>; onBackup: () => Promise<void>; onRestore: (file: File) => Promise<void>; onSteamScan: () => Promise<void>; onSteamSync: () => Promise<void>; onOAuthConnect: (provider: OAuthProvider) => Promise<void>; onTwitchOutputTest: () => Promise<void>; onReconnect: () => Promise<void>; onOpenSetup: () => void }) {
  const { t } = useI18n()
  ...
  return <main className="settings-view">
    ...
    {config.features.twitch && !status.twitchOutputPluginReady && <div className="inline-warning"><AlertTriangle size={14} /><span>{t('同時配信用プラグインを反映するためOBSを再起動してください')}</span></div>}
