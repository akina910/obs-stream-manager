    ...
    <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal" role="dialog" aria-modal="true">
        ...
        <div className="modal-body">
          ...
          <div className="field-row"><label>Steam App ID<input type="number" value={draft.library.steamAppId ?? ''} onChange={(event) => patch('library', { ...draft.library, steamAppId: event.target.value ? Number(event.target.value) : undefined })} /></label><label>インストール先<input value={draft.library.installDirectory ?? ''} onChange={(event) => patch('library', { ...draft.library, installDirectory: event.target.value || undefined, installed: Boolean(event.target.value) })} /></label></div>
          ...
          <div className="form-section"><h3>音声</h3><div className="field-row"><label>マイク (dB)<input type="number" value={draft.audio.microphoneDb} onChange={(event) => patch('audio', { ...draft.audio, microphoneDb: Number(event.target.value) })} /></label><label>ゲーム (dB)<input type="number" value={draft.audio.gameDb} onChange={(event) => patch('audio', { ...draft.audio, gameDb: Number(event.target.value) })} /></label></div><div className="field-row"><label>Discord (dB)<input type="number" value={draft.audio.discordDb} onChange={(event) => patch('audio', { ...draft.audio, discordDb: Number(event.target.value) })} /></label><label>BGM (dB)<input type="number" value={draft.audio.bgmDb} onChange={(event) => patch('audio', { ...draft.audio, bgmDb: Number(event.target.value) })} /></label></div><label>ダッキング目標 (dB)<input type="number" value={draft.audio.duckingDb} onChange={(event) => patch('audio', { ...draft.audio, duckingDb: Number(event.target.value) })} /></label></div>
          <div className="form-section"><h3>録画</h3><label>録画先<input value={draft.recording.directory} onChange={(event) => patch('recording', { ...draft.recording, directory: event.target.value })} placeholder="D:\Recordings\Game" /></label><label>リプレイ秒数<input type="number" value={draft.recording.replayBufferSeconds} onChange={(event) => patch('recording', { ...draft.recording, replayBufferSeconds: Number(event.target.value) })} /></label><div className="profile-flags"><label className="check-row"><input type="checkbox" checked={draft.recording.enabled} onChange={(event) => patch('recording', { ...draft.recording, enabled: event.target.checked })} />通常録画</label><label className="check-row"><input type="checkbox" checked={draft.recording.sourceRecord} onChange={(event) => patch('recording', { ...draft.recording, sourceRecord: event.target.checked })} />Source Record</label><label className="check-row"><input type="checkbox" checked={draft.recording.verticalRecording} onChange={(event) => patch('recording', { ...draft.recording, verticalRecording: event.target.checked })} />Aitum Vertical録画</label></div></div>
